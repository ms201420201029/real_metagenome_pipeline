...
group <- read.table(group,header=FALSE,row.names=1,sep='\t',check.names=F,quote="")
...
#}else{
pdf(outfilepdf,height=9,width=10)
pdf_width
temp.mar <- par("mar")
#temp.mar[4] <- temp.mar[4]+mar_r
temp.mar[1] <- temp.mar[1]+mar_r*0.8
par(mar=temp.mar)
temp.mar
temp <- barplot(as.matrix(data),col=gcolor,font=2,space=spa,xaxt='n')
text(temp,y = -y.value,srt=srt,labels=colnames(data),xpd=T)
mtext('Relative Abundance',side=2,line=3,cex=1.1,font=2)
mtext(paste0("ARDB ",title.infor," Abundance barplot"), side=3, line=1, cex=1.1,font=2)

legend(x=8,y=-y.value*2,pch=15, col=gcolor,legend=rownames(data), bty="n",
       pt.cex=pt.cex, cex=1, x.intersp=1.2, y.intersp=2,xpd=TRUE,ncol=4)

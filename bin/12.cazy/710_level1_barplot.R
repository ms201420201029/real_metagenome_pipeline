...
#mtext('Count',side=2,line=3,cex=1.1,font=2)
#mtext(paste0("Class"," Abundance barplot"), side=3, line=1, cex=1.1,font=2)
mtext(paste0(args[3]," Abundance barplot"), side=3, line=1, cex=1.1,font=2)
...
temp <- barplot(as.matrix(data),col=gcolor,font=2,space=spa,xaxt='n')
text(temp,y = -y.value,srt=srt,labels=colnames(data),xpd=T)
mtext('Relative Abundance*10000',side=2,line=3,cex=1.1,font=2)
mtext(paste0(args[3]," Abundance barplot"), side=3, line=1, cex=1.1,font=2)

legend(x=5,y=-y.value*1.5,pch=15, col=gcolor,legend=rownames(data), bty="n",
       pt.cex=pt.cex, cex=1, x.intersp=1.2, y.intersp=2,xpd=TRUE,ncol=6)

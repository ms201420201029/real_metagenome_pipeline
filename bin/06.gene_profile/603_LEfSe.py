...
if __name__ == '__main__':
    ...
    os.system("awk '$4>%s' %s/LDA.res >%s/diff.marker.filter.txt.tmp"%(params['LDA'],params['out_dir'],params['out_dir']))
    with open("%s/diff.marker.filter.txt.tmp"%params['out_dir']) as fqin,open("%s/diff.marker.filter.tsv"%params['out_dir'],"w") as fqout:
        for line in fqin:
            tabs = line.strip().split("\t")
            fqout.write("%s\t%s\t\t%s\t\t%s\n"%(tabs[0],tabs[1],tabs[4],tabs[2]))
    os.system("rm %s/diff.marker.filter.txt.tmp"%(params['out_dir']))
    os.system("cut -f 1 %s/diff.marker.filter.tsv |while read line;do echo \"grep \'$line\' %s >> %s/diff.marker.filter.profile.tsv\";done>%s/do.sh"%(params['out_dir'],params['infile'],params['out_dir'],params['out_dir']))
    os.system("head -1 %s > %s/diff.marker.filter.profile.tsv"%(params['infile'],params['out_dir']))
    os.system("sh %s/do.sh"%params['out_dir'])
    os.system("rm %s/do.sh"%params['out_dir'])

...
def read_params(args):
    ...
    parser.add_argument('-d', '--database', dest='database', metavar='FILE', type=str, default="/data_center_10/Project/RY2017L01C01-1-tmp/02.taxon_metaphlan/database.txt",
...
if __name__ == '__main__':
    ...
    with open(profile_table_file,"r") as fqin:
        for line in fqin:
            ...
            try:
                abun[tabs[0]] =float(tabs[1])/float(100)
    ...
    with open(database_file) as fqin , open("%s/test.tax"%out_dir,"w") as fqout:
        for line in fqin:
            ( k, p, c, o, f, g, s,_,_) = line.strip().split("\t")
            if top.has_key(s):
                s_out = re.sub(":","",s)
                s_out = re.sub("\.","",s_out)
                g_out = re.sub("--","--g",g)

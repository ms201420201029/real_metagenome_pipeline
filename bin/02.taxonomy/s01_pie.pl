...
sub pie_chart {
	...
	foreach $tick_name (sort {$abun_p->{$b} <=> $abun_p->{$a}} keys %$abun_p){
		...
		$svg -> text('id' => "$tick_name\_tick_name",
			     ...
			     'style' => {'font-family' => 'Courier New', 'font-size' => $font_size, 'text-anchor' => $direction}) -> cdata($tick_name) if $count_ticks < $ticks_num;
		$rad += $abun * pi;
		$count_ticks ++;
		$abun_other -= $abun;
	}
...
#	$tick_name = "Other";
	...
	$svg -> text('id' => "$tick_name\_other",
		     'x' => $centre_x + cos($rad) * ($pie_radius + 1.1 * $ticks_length),
		     'y' => $centre_y - sin($rad) * ($pie_radius + 1.1 * $ticks_length) + $tick_name_height / 2,
		     'style' => {'font-family' => 'Courier New', 'font-size' => $font_size, 'text-anchor' => $direction}) -> cdata("Other");

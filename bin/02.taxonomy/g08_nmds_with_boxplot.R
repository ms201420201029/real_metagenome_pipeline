...
Nmds <- function(dist.data, sample.list, method){
	...
	par(mar = c(3, 5, 2, 5));
	boxplot(nmds$points[,1] ~ sample.list, pch = 20, col = col.sample[sample.list], notch = F, horizontal = T, cex.axis = 0.8);
	par(mar = c(5, 2, 5, 3));
	boxplot(nmds$points[,2] ~ sample.list, pch = 20, col = col.sample[sample.list], notch = F, cex.axis = 0.8);
	dev.off();
}

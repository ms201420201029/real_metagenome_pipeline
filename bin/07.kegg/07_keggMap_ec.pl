...
my ($ecmap, $diff, $outdir, $help, $mapdir, $species);

GetOptions("species:s" => \$species, "mapdir:s" => \$mapdir, "ecmap:s" => \$ecmap, "diff:s" => \$diff, "outdir:s" => \$outdir, "help|?" => \$help);
$outdir ||= ".";
our ($ecMap, $mapDir);
do "$Bin/keggConf.pl";
$ecMap2=$ecMap;
$ecMap = $ecmap if (defined $ecmap);
...
options:
	-ecmap: ec_map.tab file, default is "$ecMap2";
        -mapdir <path>  map derectory;
        -diff <file> *  *.EC.Diff.xls, "EC\\t\\t\\t\\t\\tlog2";
        -outdir <path>  output directory, default is current directory "."
        -help|?         help information
e.g.:
        perl $0 -diff A.EC.diff.xls -outdir . -species sce -ecmap /data_center_02/Database/KEGG/20141209/EC_map.tab  -mapdir /ifs2/BC_MG/PROJECT/RNA/Database/Saccharomyces_cerevisiae_S288C/ko_go_sgd/map/

USAGE
}


if (!defined $diff || defined $help) {
	&usage();
	exit 1;
}
...
while (<DIFF>) {
	...
	$ec=$temp[0];
	next unless (exists $maps{$ec});
	for $mapId (@{$maps{$ec}}) {
		$mapImg = "$pre$mapId.png";
		next if (! -f "$mapDir/$mapImg");
		open CONF, "< $mapDir/$pre$mapId.html" || die $!;
		while (<CONF>) {
			if (/rect\s[^\d]*([\d]+),([\d]+)[^\d]*([\d]+),([\d]+).*K\d+\+$ec*/) {  ##### wuchunyan at 20131014
				push @{$data{$mapImg}}, [$temp[0], $1, $2, $3, $4, $color, "rect"]; # map => [[gene, x1, y1, x2, y2, color], ...]
				$htmlData{"$pre$mapId"}{"$1,$2,$3,$4"}{$color}{$temp[0]} = $ratios{$temp[0]};  ##### wuchunyan at 20131014
			}elsif (/poly\s[^\d]*([\d]+),([\d]+)[^\d]*([\d]+),([\d]+),([\d]+),([\d]+),([\d]+),([\d]+).*K\d+\+$ec*/){ ##### wuchunyan at 20131014
				push @{$data{$mapImg}}, [$temp[0], $1, $2, $3, $4, $5, $6, $7, $8, $color, "poly"];
				$htmlData{"$pre$mapId"}{"$1,$2,$3,$4,$5,$6,$7,$8"}{$color}{$temp[0]} = $ratios{$temp[0]}; ##### wuchunyan at 20131014
			}elsif (/poly\s[^\d]*([\d]+),([\d]+)[^\d]*([\d]+),([\d]+),([\d]+),([\d]+).*K\d+\+$ec*/){    ##### wuchunyan at 20131014
				push @{$data{$mapImg}}, [$temp[0], $1, $2, $3, $4, $5, $6, $color, "poly"];
				$htmlData{"$pre$mapId"}{"$1,$2,$3,$4,$5,$6"}{$color}{$temp[0]} = $ratios{$temp[0]};  ##### wuchunyan at 20131014
			}
		}
		close CONF;
	}
...
#		$im->string(gdSmallFont, $b->[1], $b->[2] + 2, $b->[0], $black);
	...
	for $rect (keys %{$htmlData{$map}}) {
		if (exists $htmlData{$map}{$rect}{"red"} || exists $htmlData{$map}{$rect}{"green"}){
			...
			if (exists $htmlData{$map}{$rect}{"red"}) {
				...
				for $ec (sort {&max(\%{$htmlData{$map}{$rect}{"red"}{$b}}) <=> &max(\%{$htmlData{$map}{$rect}{"red"}{$a}})} keys %{$htmlData{$map}{$rect}{"red"}}) {
					$temp .= "<li>$ec:$htmlData{$map}{$rect}{'red'}{$ec}<\/li>";
				}
				$temp .= "</ul></li>";
			}
			if (exists $htmlData{$map}{$rect}{"green"}) {
				$temp .= "<li style=\\\"color: #0f0;\\\">Down regulated<ul>";
				for $ec (sort {&max(\%{$htmlData{$map}{$rect}{"green"}{$b}}) <=> &max(\%{$htmlData{$map}{$rect}{"green"}{$a}})} keys %{$htmlData{$map}{$rect}{"green"}}) {
					$temp .= "<li>$ec:$htmlData{$map}{$rect}{'green'}{$ec}<\/li>";
				}
				$temp .= "</ul></li>";
			}
			$temp .= "</ul>";

...
#*****************************************************************************************************
#*****************************************************************************************************
detect_differentially_abundant_features <- function(jobj, g, output, pflag = NULL, threshold = NULL, B = NULL,groupname = NULL){
  ...
  colnames(Total_matrix) = c("ko_num",paste0("Mean(",groupname[1],")"),paste0("Variance(",groupname[1],")"),paste0("stdErr(",groupname[1],")"),paste0("Mean(",groupname[2],")"),paste0("Variance(",groupname[2],")"),paste0("stdErr(",groupname[2],")"),"p_value","q_value")
#  write(t(Total_matrix), file = output, ncolumns = 9, sep = "\t" );
  write.table(Total_matrix, file = output, sep = "\t" ,row.names=F,col.names=T,quote=F);

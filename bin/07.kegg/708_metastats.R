...
tt <- detect_differentially_abundant_features(jobj, gstart,paste0(outputdir,"metastats_result.txt"),pflag= pflag, threshold = cutoff_p,B=5,groupname=group_names)
result <- cbind(tt[,c(1,2,5,8,9)],apply(tt,1,function(x) if(as.numeric(x[2])>as.numeric(x[5])){group_names[1]}else{group_names[2]}))
group1_title = paste0("mean(",group_names[1],")")
group2_title = paste0("mean(",group_names[2],")")

colnames(result)<- c("ko_num",group1_title,group2_title,"pvalue","fdr","enrichment")
write.table(result,file=paste0(outputdir,"diff.marker.tsv"),sep="\t",quote=F,row.names=F,col.names=T)
result_filter <- result[which(tt[,8]<cutoff_p),]
write.table(result_filter,file=paste0(outputdir,"diff.marker.filter.tsv"),sep="\t",quote=F,row.names=F,col.names=T)
...
ggplot(data=data2,aes(x=variable,y=value))+geom_boxplot(aes(fill=group))+
  ...
  scale_fill_manual(values=cols_brewer[1:2])+labs(x="KO number",y="abundance")
#}
dev.off()

...
pdf("$prefix.class.pdf",width=9, height=8) 
qplot(Pathway,precent,data = counts, geom = 'bar')+coord_flip()+geom_bar(aes(fill=CLASS),stat="identity")+labs(title="KEGG Classification",y="Percent of Genes (%)",x="")+theme(axis.text=element_text(colour="black",size=11))+geom_text(label=counts\$num,size=3,hjust=-0.5, vjust=0.5)+ylim(0,20)
dev.off()
RTXT

mycorr = function (corr, method = c("circle", "square", "ellipse", "number", 
          ...
          uppCI.mat = NULL, ...) 
{
  ...
  if (is.null(addgrid.col)) {
    addgrid.col <- ifelse(method == "color" | method == 
                            "shade", "white", "grey")
  }
  ...
  int <- intercept * zoom
  if (min(corr) < -1 - .Machine$double.eps || max(corr) > 
  ...
  if (!add) {
    ...
    for (i in 1:50) {
      xlim <- c(m1 - 0.5 - xlabwidth, m2 + 0.5 + mm * 
                  cl.ratio * (cl.pos == "r"))
      ylim <- c(n1 - 0.5 - nn * cl.ratio * (cl.pos == 
                                              "b"), n2 + 0.5 + ylabwidth)
      plot.window(xlim + c(-0.2, 0.2), ylim + c(-0.2, 
                                                0.2), asp = 1, xaxs = "i", yaxs = "i")
      x.tmp <- max(strwidth(newrownames, cex = tl.cex))
      y.tmp <- max(strwidth(newcolnames, cex = tl.cex))
      if (min(x.tmp - xlabwidth, y.tmp - ylabwidth) < 
            1e-04) 
        break
      xlabwidth <- x.tmp
      ylabwidth <- y.tmp
    ...
    cat(ylim)
    cat("\n")
    #xlim=c(-7.896025,32.7)
    #ylim=c(0.35,10.986973)
  ...
  if (!is.null(p.mat) & !insig == "n") {
    ...
    if (insig == "pch") {
      points(pos.pNew[, 1][ind.p], pos.pNew[, 2][ind.p], 
             pch = pch, col = pch.col, cex = pch.cex, lwd = 3)
    }
    if (insig == "p-value") {
...
mycolorlegend = function (colbar, labels, at = NULL, xlim = c(0, 1), ylim = c(0,
  ...
  align = c("c", "l", "r"), addlabels = TRUE, ...)
{
  ...
  if (vertical) {
    ...
    rect(xxx[1:len],rep(ylim[1], len), xxx[-1], rep(ylim[1] + ygap *
      rat1, len), col = colbar, border = colbar)
    rect(xlim[1], ylim[1], xlim[2],ylim[1] + ygap * rat1,
      border = "black")
    pos.xlabel <- rep(xlim[1] + xgap * max(rat2, rat1),
      length(at))
    segments( at,ylim[1] + ygap * rat2[1], at, ylim[1] + ygap * rat2[2])
    if (addlabels) {
      if (align == "l")
        text(x = pos.xlabel, y = at, labels = labels,
          pos = 4, ...)
      if (align == "c")
        text(x=at, y = (ylim[1] + ygap * rat2[1])*2, labels = labels,
          ...)
      if (align == "r")
  ...
  if (!vertical) {
    ...
    segments(at, ylim[2] - ygap * rat2[1], at, ylim[2] -
      ygap * rat2[2])
    if (addlabels) {
      switch(align, l = text(at, pos.ylabel, labels, pos = 1,
        ...), c = text(at, (pos.ylabel + ylim[1])/2,
        labels = labels, ...), r = text(at, ylim[1],
        labels = labels, pos = 2, ...))
    }
  }
}
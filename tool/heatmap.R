...
top=30
data <- read.table(profile,check.names = F,sep="\t",quote="",header = T,row.names = 1)
if(nrow(data)>=top){
  data <- data[order(apply(data,1,sum),decreasing = T)[1:top],]
}
write.table(data,file=paste0(outdir,"for_plot_filter.txt"),sep="\t",quote=F)  

group=read.table(groupfile,sep="\t",header=F,row.names=1,check.names=F,quote="")
...
library(gplots)
pdf(paste0(outdir,"ko.heatmap.pdf"),12,16)
if(ncol(data)<=2){
  plot(0,type="n")

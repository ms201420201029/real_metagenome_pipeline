...
if __name__ == '__main__':
    ...
    pdf_file = params['out_dir'] +"/"+use_method+ '_adonis_pca.pdf'
    png_file = params['out_dir'] +"/"+use_method+ '_adonis_pca.png'
    env = Environment(loader=FileSystemLoader(tool_default_dir),autoescape=False)
    template = env.get_template("t12_adonis_pca.R")
    Rtxt = template.render(tool_default_dir = tool_default_dir,\
                           profile_table =params['profile_table'],\
                           group_file =params['group_file'],\
                           pdf_file = pdf_file,\
                           method = use_method)
    with open("%s/adonis_pca.R" % params["out_dir"],"w") as fqout:
        fqout.write(Rtxt)
    Rrun("%s/adonis_pca.R"%params["out_dir"])

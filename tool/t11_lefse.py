...
def get_commands(infile, LEfSe_path, out_dir, LDA, c, u):
    ...
    commands = []
    commands.append('%s %sformat_input.py %s %s/LDA.in -c %s -u %s -o 1000000' % (python_version, LEfSe_path, infile, out_dir, c, u))
    commands.append('source /home/huangy/.bashrc')
    commands.append('%s %srun_lefse.py %s/LDA.in %s/LDA.res -l %s' % (python_version, LEfSe_path, out_dir, out_dir, LDA))
    commands.append('%s %splot_res.py %s/LDA.res %s/LDA.pdf --format pdf --dpi 150' % (python_version, LEfSe_path, out_dir, out_dir))
    commands.append('%s %splot_res.py %s/LDA.res %s/LDA.png --format png --dpi 150' % (python_version, LEfSe_path, out_dir, out_dir))
    commands.append('source ~/.bashrc')

    commands.append("awk '$4>%s' %s/LDA.res >%s/diff.marker.filter.txt" % (params['LDA'], out_dir, out_dir))
    commands.append("cut -f 1 %s/diff.marker.filter.txt > %s/rows.txt" % (out_dir, out_dir))
    commands.append("Rscript /data_center_01/pipeline/real_metagenome/real_metagenome_v1.0.0/tool/cut_rows_cols.R %s %s/rows.txt %s %s/diff.marker.filter.profile.txt" % (params['infile'], out_dir, params['groupDir'], out_dir))
    commands.append("#rm %s/rows.txt" % out_dir)
...
if __name__ == '__main__':
    ...
    with open(params['out_dir'] + '/commands.sh', 'w') as fp:
        fp.write('\n'.join(commands))
    shfile = params['out_dir'] + '/commands.sh'
